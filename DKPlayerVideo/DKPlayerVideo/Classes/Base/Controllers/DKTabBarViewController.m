...
- (void)viewWillAppear:(BOOL)animated{
    ...
    for (int i = 0; i < self.tabBar.items.count; i++) {
        UITabBarItem *v = self.tabBar.items[i];
        NSLog(@"%@",v);
    }
...
-(void)setupChildViewController:(UIViewController *)vc Title:(NSString *)title Image:(UIImage *)img andSelectedImg:(UIImage *)selectedImg {
    ...
    if ([vc isKindOfClass:[DKVideoViewController class]]) {
        nav = [UINavigationController rootVC:vc translationScale:NO];
        nav.gk_openScrollLeftPush = YES; // 开启左滑push功能
    }else{
        nav = [[DKNavigationController alloc] initWithRootViewController:vc];
    }
